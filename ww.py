...
def card_selection(n_people, target, threshold, forced_roles, black_listed,
                   players):  # argv takes optional string arguments for specified cards we want
    ...
    while deck_points not in target_range:
        ...
        while (len(deck) < n_people):
            ...
            if deck.count(role) == role_info[
                'limit']:  # if the # unique roles in the deck are equal too the role limit, then we can can not add this card
            ...
            for inclusions in card_inclusions[
                role]:  # inclusions is a nested list, card_inclusions[role] is a list of lists
